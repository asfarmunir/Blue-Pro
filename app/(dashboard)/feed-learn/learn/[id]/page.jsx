...
const Page = ({ params: { id } }) => {
  ...
  const getPostData = async () => {
    try {
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/learn/${id}`
      );
      const data = res.data;
  ...
  const addLike = async () => {
    startAddingLike(async () => {
      try {
        await axios.post(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/learn/likeDislikeLearn`,
          {
            id: data.user._id,
            postId: postData._id,
          }
  ...
  const addComment = async () => {
    startAddingComment(async () => {
      try {
        await axios.post(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/learn/addComment`,
          {
            id: data.user._id,
            userName: "Admin",
            userImage:
